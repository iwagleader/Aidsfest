...
if(("ziptie" call INV_GetItemAmount < 1) && ("ziptie_bh" call INV_GetItemAmount < 1)) exitwith {player groupchat "You need a zip tie in your inventory in order to use this function."};
...
if(("ziptie_bh" call INV_GetItemAmount > 0) && (_ziptiedciv == 0)) then{
["ziptie_bh", -1] call INV_AddInvItem;
(format['server globalchat "%3 was zip tied by Bounty Hunter %2";if (rolestring == "%1") then {execVM "Unziptie.sqf";}', _civ, name player, name _civ]) call broadcast;
};
