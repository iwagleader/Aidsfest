...

{
	...
	case 10:
	{
		...
		if ( _uid == "76561198084017816") exitWith {player sideChat "My name is Lt. Buffalo and I don't respect developers. In fact I kind of acted like an asshole so I lost my checkpoint builder. <3 Genovah";};
	...
	if(INV_shortcuts)then{INV_shortcuts=false; titletext["RL keys off", "PLAIN DOWN"];[] execVM "actions\actionsRemove.sqs"}else{INV_shortcuts=true; titletext["RL keys on", "PLAIN DOWN"];[] execVM "actions\copactions.sqf"};};
	//L key
	case 38: 
	...
	if (iscop) then {
		player sideChat "----- FUNCTION DISABLED ------";
	};
