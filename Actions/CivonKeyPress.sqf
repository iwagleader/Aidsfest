...

{
	...
	if(INV_shortcuts)then{INV_shortcuts=false; titletext["RL keys off", "PLAIN DOWN"];[] execVM "actions\actionsRemove.sqs"}else{INV_shortcuts=true; titletext["RL keys on", "PLAIN DOWN"];[] execVM "actions\civactions.sqf"};};
	//L key
	case 38: 
  ...
  //tilde key
	...
	if(isEngineOn _vcl and !isnil "_lvl") then
        ...
        if(_lvl == 6)then{_vcl setVelocity [(_vel select 0) * 1.018, (_vel select 1) * 1.018, (_vel select 2) * 0.99]};			
       	if(_lvl == 7)then{_vcl setVelocity [(_vel select 0) * 1.020, (_vel select 1) * 1.020, (_vel select 2) * 0.99]};			
